...
(() => {
  ...
  function drawPlayer() {
    // Chester — yellow-raincoat hooded kid (matches L2/L3 style)
    ctx.save();
    ctx.translate(-cam.x, -cam.y);
    ...
    ctx.beginPath(); ctx.ellipse(cx, py + ph + 2, 11, 4, 0, 0, Math.PI*2); ctx.fill();
    ...
    // facing pip (torch indicator)
    ctx.fillStyle = '#fff1c4';
    ctx.beginPath();
    ctx.arc(cx + Math.cos(player.facing) * 11, cy + Math.sin(player.facing) * 11, 2.5, 0, Math.PI*2);
    ctx.fill();
    // Key indicator floating above player
    if (state.keys > 0) {
      ctx.fillStyle = '#e6e8f2';
      ctx.font = '700 12px system-ui, sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText('🗝\u00d7' + state.keys, cx, py - 6);
    }
    ctx.restore();
  }
  ...
  function drawMonster(m, type) {
    ...
    if (type === 'expression') {
      // Ex Preshon — matches child-drawing style (peach-orange, tall ear-peaks,
      // crosshatch fur, ONE big black scribbled eye, arms raised, jagged tattered bottom)
      const ex = m.x, ey = m.y, ew = m.w, eh = m.h;
      const t = performance.now() / 1000;
      const sway = Math.sin(t * 2) * 1.2;
      // shadow
      ctx.fillStyle = 'rgba(0,0,0,0.55)';
      ctx.beginPath(); ctx.ellipse(ex + ew/2, ey + eh + 3, 20, 5, 0, 0, Math.PI*2); ctx.fill();

      const bodyTop = ey + 14;
      const bodyBot = ey + eh;
      // main peach-orange body
      const bodyFill = expression.hurtFlash > 0 ? '#ffd9a0' : '#f59e3a';
      ctx.fillStyle = bodyFill;
      ctx.beginPath();
      ctx.moveTo(ex - 2, bodyTop);
      ctx.lineTo(ex + ew + 2, bodyTop);
      // jagged tattered bottom edge
      const teeth = 7;
      for (let i = teeth; i >= 0; i--) {
        const tx = ex - 2 + (ew + 4) * (i / teeth);
        const ty = bodyBot - (i % 2 === 0 ? 0 : 6);
        ctx.lineTo(tx, ty);
      }
      ctx.closePath();
      ctx.fill();
      // darker orange shading
      ctx.fillStyle = '#d97e1e';
      ctx.fillRect(ex - 1, bodyTop + 2, 4, eh - 18);
      // crosshatch fur texture
      ctx.strokeStyle = 'rgba(90, 40, 5, 0.45)';
      ctx.lineWidth = 0.7;
      for (let r = 0; r < 4; r++) {
        const yy = bodyTop + 6 + r * 8;
        ctx.beginPath();
        for (let x = 0; x < ew; x += 3) {
          ctx.moveTo(ex + x, yy);
          ctx.lineTo(ex + x + 2, yy + 2);
        }
        ctx.stroke();
      }
      // body outline
      ctx.strokeStyle = '#7a3a05';
      ctx.lineWidth = 1.8;
      ctx.beginPath();
      ctx.moveTo(ex - 2, bodyTop);
      ctx.lineTo(ex + ew + 2, bodyTop);
      ...
      // Two raised clawed arms
      ctx.strokeStyle = '#c56b10';
      ctx.lineWidth = 3.5;
      ctx.lineCap = 'round';
      const laShoulderX = ex + 1, laShoulderY = bodyTop + 4;
      const laHandX = ex - 10 + sway, laHandY = ey + 2;
      ctx.beginPath();
      ctx.moveTo(laShoulderX, laShoulderY);
      ctx.quadraticCurveTo(ex - 6, ey + 10, laHandX, laHandY);
      ctx.stroke();
      const raShoulderX = ex + ew - 1, raShoulderY = bodyTop + 4;
      const raHandX = ex + ew + 10 - sway, raHandY = ey + 2;
      ctx.beginPath();
      ctx.moveTo(raShoulderX, raShoulderY);
      ctx.quadraticCurveTo(ex + ew + 6, ey + 10, raHandX, raHandY);
      ctx.stroke();
      // Clawed hands (3 spiky fingers each)
      ctx.strokeStyle = '#3a1a05';
      ctx.lineWidth = 1.5;
      for (const [hx0, hy0, dir] of [[laHandX, laHandY, -1], [raHandX, raHandY, 1]]) {
        ctx.fillStyle = '#f59e3a';
        ctx.beginPath(); ctx.arc(hx0, hy0, 3, 0, Math.PI*2); ctx.fill();
        ctx.stroke();
        for (let f = -1; f <= 1; f++) {
          ctx.beginPath();
          ctx.moveTo(hx0, hy0 - 1);
          ctx.lineTo(hx0 + dir * (2 + f * 0.5), hy0 - 4 - Math.abs(f));
          ctx.stroke();
        }
      }

      // Head (orange round blob)
      const hx = ex + ew/2, hy = ey + 10;
      ctx.fillStyle = bodyFill;
      ctx.beginPath();
      ctx.ellipse(hx, hy, ew/2 + 2, 11, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.strokeStyle = '#7a3a05';
      ctx.lineWidth = 1.8;
      ctx.stroke();

      // TWO tall flame-like ear-peaks on top of head
      ctx.fillStyle = bodyFill;
      ctx.strokeStyle = '#7a3a05';
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(hx - 7, hy - 6);
      ctx.quadraticCurveTo(hx - 10, hy - 16, hx - 6, hy - 18);
      ctx.quadraticCurveTo(hx - 3, hy - 12, hx - 2, hy - 7);
      ctx.closePath();
      ctx.fill(); ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(hx + 2, hy - 7);
      ctx.quadraticCurveTo(hx + 5, hy - 16, hx + 8, hy - 17);
      ctx.quadraticCurveTo(hx + 11, hy - 10, hx + 7, hy - 6);
      ctx.closePath();
      ctx.fill(); ctx.stroke();

      // ONE huge black scribbled eye (center of face)
      const pulse = 0.7 + Math.sin(t * 4) * 0.3;
      const glow = expression.eyeGlow != null ? expression.eyeGlow : 1;
      if (glow > 0) {
        ctx.save();
        ctx.shadowColor = '#ff9a3a';
        ctx.shadowBlur = 18 * pulse * glow;
        ctx.fillStyle = 'rgba(255,170,60,0.45)';
        ctx.beginPath();
        ctx.arc(hx, hy + 1, 7 * glow, 0, Math.PI*2);
        ctx.fill();
        ctx.restore();
        ctx.fillStyle = '#0a0604';
        ctx.beginPath();
        ctx.ellipse(hx, hy + 1, 5.5, 4.8, 0, 0, Math.PI*2);
        ctx.fill();
        ctx.strokeStyle = '#0a0604';
        ctx.lineWidth = 0.8;
        for (let s = 0; s < 5; s++) {
          ctx.beginPath();
          ctx.moveTo(hx - 6 + s * 0.5, hy - 3 + s);
          ctx.lineTo(hx + 6 - s * 0.3, hy + 4 - s);
          ctx.stroke();
        }
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(hx - 1.5, hy - 0.5, 0.9, 0, Math.PI*2);
        ctx.fill();
      } else {
        // Closed eye (blink)
        ctx.strokeStyle = '#0a0604';
      ...
      // EXLENA — pink daycare-teacher monster. Child-drawn style: wobbly outlines,
      // stripey dress, horned head, huge ringed eyes, toothy grin. NOT scared of torch.
      const recoil = exlena.state === 'recoil';
      const bodyFill = recoil ? '#ffb0d0' : '#ff5fa2';
      const bodyStroke = '#6a1a3d';
      const t = performance.now() / 1000;
      const wob = Math.sin(t * 3) * 0.6; // gentle wobble
      // shadow
      ctx.fillStyle = 'rgba(0,0,0,0.5)';
      ctx.beginPath(); ctx.ellipse(m.x + m.w/2, m.y + m.h + 2, 18, 5, 0, 0, Math.PI*2); ctx.fill();
      ctx.lineWidth = 1.6;
      // Rounded pear body (slightly wobbly control points)
      ctx.fillStyle = bodyFill;
      ctx.strokeStyle = bodyStroke;
      ctx.beginPath();
      ctx.moveTo(m.x + 4 + wob, m.y + m.h);
      ctx.quadraticCurveTo(m.x - 2 - wob, m.y + m.h/2, m.x + 3, m.y + 10);
      ctx.quadraticCurveTo(m.x + m.w/2, m.y - 2 + wob*0.5, m.x + m.w - 3, m.y + 10);
      ctx.quadraticCurveTo(m.x + m.w + 2 + wob, m.y + m.h/2, m.x + m.w - 4 - wob, m.y + m.h);
      ctx.closePath();
      ctx.fill(); ctx.stroke();
      // Stripey dress texture (crayon stripes)
      ctx.strokeStyle = 'rgba(106, 26, 61, 0.4)';
      ctx.lineWidth = 1;
      for (let i = 0; i < 4; i++) {
        const yy = m.y + 20 + i * 5;
        ctx.beginPath();
        ctx.moveTo(m.x + 3, yy);
        ctx.quadraticCurveTo(m.x + m.w/2, yy + 1.5, m.x + m.w - 3, yy);
        ctx.stroke();
      }
      // Two small black horns on top of head
      ctx.fillStyle = '#14101a';
      ctx.beginPath();
      ctx.moveTo(cx - 7, m.y + 4);
      ctx.lineTo(cx - 11, m.y - 6);
      ctx.lineTo(cx - 3, m.y + 2);
      ctx.closePath();
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(cx + 7, m.y + 4);
      ...
      ctx.lineWidth = 1.2;
      ctx.beginPath(); ctx.arc(cx - 6, m.y + 14, 5.5, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
      ctx.beginPath(); ctx.arc(cx + 6, m.y + 14, 5.5, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
      // Ringed pupils
      ctx.strokeStyle = '#14101a';
      ctx.lineWidth = 1.6;
      ctx.beginPath(); ctx.arc(cx - 6, m.y + 14, 3, 0, Math.PI * 2); ctx.stroke();
      ctx.beginPath(); ctx.arc(cx + 6, m.y + 14, 3, 0, Math.PI * 2); ctx.stroke();
      ctx.fillStyle = '#14101a';
      ctx.beginPath(); ctx.arc(cx - 6, m.y + 14, 1.5, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(cx + 6, m.y + 14, 1.5, 0, Math.PI * 2); ctx.fill();
      // Wide toothy grin
      ctx.fillStyle = '#3a0820';
      ctx.beginPath();
      ctx.ellipse(cx, m.y + 26, 9, 5, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#ffffff';
      ctx.strokeStyle = '#6a1a3d';
      ctx.lineWidth = 0.8;
      for (let i = -3; i <= 3; i++) {
        const tx = cx + i * 2.5;
        ctx.beginPath();
        ctx.moveTo(tx - 1, m.y + 24);
        ctx.lineTo(tx + 1, m.y + 24);
        ctx.lineTo(tx, m.y + 28);
        ctx.closePath();
        ctx.fill(); ctx.stroke();
      }
      // Clawed arms
      ...
      ctx.lineWidth = 1.2;
      ctx.beginPath();
      ctx.moveTo(m.x + 2, m.y + 22);
      ctx.lineTo(m.x - 4, m.y + 30);
      ctx.lineTo(m.x - 2, m.y + 34);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(m.x + m.w - 2, m.y + 22);
      ctx.lineTo(m.x + m.w + 4, m.y + 30);
      ctx.lineTo(m.x + m.w + 2, m.y + 34);
      ctx.stroke();
    }
    ctx.restore();
  }
