...
(() => {
  ...
  function drawPlayer() {
    ...
    // Yellow raincoat body (hooded kid)
    ...
    // facing pip (flashlight indicator)
  ...
  function drawExpression() {
    ...
    ctx.save();
    ctx.translate(-cam.x, -cam.y);
    const ex = expression.x, ey = expression.y, ew = expression.w, eh = expression.h;
    ...
    // Tattered orange body — kid-drawn robe with jagged bottom
    const baseX = ex + ew/2;
    const bodyTop = ey + 14;
    const bodyBot = ey + eh;
    // main orange body fill
    ...
    ctx.closePath();
    ctx.fill();
    // darker orange shading down the middle-left for depth
    ctx.fillStyle = '#d97e1e';
    ctx.fillRect(ex - 1, bodyTop + 2, 4, eh - 18);
    // body outline (hand-drawn look)
    ctx.strokeStyle = '#7a3a05';
    ctx.lineWidth = 1.8;
    ctx.stroke();

    // Two raised clawed arms (waving up and out)
    ...
    ctx.stroke();
    // Clawed hands (3 tiny fingers each)
    ...
    // TWO wavy flame-like peaks on top of head (like Ex Preshon's drawing)
    ctx.fillStyle = '#f59e3a';
    ctx.strokeStyle = '#7a3a05';
    ctx.lineWidth = 1.5;
    // left peak
    ...
    ctx.quadraticCurveTo(hx - 10, hy - 14, hx - 6, hy - 16);
    ...
    ctx.quadraticCurveTo(hx + 5, hy - 14, hx + 8, hy - 15);
    ...
    const glow = expression.eyeGlow;
    if (glow > 0) {
      // orange glow halo behind eye (so player can still spot him in dark)
      ctx.save();
      ...
      ctx.fill();
      ctx.restore();
      // big black scribbled eye
      ctx.fillStyle = '#0a0604';
      ctx.beginPath();
      ctx.ellipse(hx, hy + 1, 5.5, 4.8, 0, 0, Math.PI*2);
      ctx.fill();
      // scribble lines over the eye (kid-drawn)
      ...
      // closed eye line (blinking)
      ctx.strokeStyle = '#0a0604';
      ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(hx - 5, hy + 1); ctx.lineTo(hx + 5, hy + 1); ctx.stroke();
    }

    // STATE marker (subtle ring when frozen)
    if (expression.state === 'frozen') {
      ctx.strokeStyle = 'rgba(155,231,255,0.7)';
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(hx, ey + eh/2, ew/2 + 8, 0, Math.PI*2);
      ctx.stroke();
    } else if (expression.state === 'recoil') {
      // Yellow warning arc — flinching from light
      ctx.strokeStyle = 'rgba(255,220,110,0.55)';
      ctx.lineWidth = 2;
      ctx.setLineDash([4, 3]);
      ctx.beginPath();
      ctx.arc(hx, ey + eh/2, ew/2 + 10, 0, Math.PI*2);
      ctx.stroke();
      ctx.setLineDash([]);
    }
    // Hurt flash (white halo when torch hits him)
    if (expression.hurtFlash > 0) {
      ctx.save();
      ctx.globalAlpha = expression.hurtFlash;
      ctx.fillStyle = 'rgba(255,255,220,0.55)';
      ctx.beginPath();
      ctx.arc(hx, ey + eh/2, ew/2 + 14, 0, Math.PI*2);
      ctx.fill();
      ctx.restore();
    }
    ctx.restore();
